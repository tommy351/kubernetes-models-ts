module github.com/tommy351/kubernetes-models-ts/third-party/argo-cd

go 1.26.2

// `k8s.io/kubernetes` (transitively required via argo-cd → gitops-engine)
// declares `replace ... => ./staging/...` entries, which propagate to
// consumers as `v0.0.0` placeholder versions. Pin each staging module so
// `go mod tidy` resolves real versions instead.
replace (
...
tool github.com/argoproj/argo-cd/v3/pkg/apis/application/v1alpha1
...
require (
	...
	k8s.io/kubectl v0.36.0-alpha.0 // indirect
